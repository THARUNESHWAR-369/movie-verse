export const TitleCard = ({
  ...
  return (
    <div className="title-card text-white flex flex-col gap-4">
      <div className="title-card-h1 w-[100%]">
        <h1 className="title-card-h1 text-6xl tracking-wide font-bold">{movieTitle}</h1>
      </div>
        ...
        <ul className="flex gap-3">
          {movieGenres &&
            movieGenres.map((genre, index) => (
              <li key={index} className="title-card-genre-li">
                {genre}
              </li>
            ))}
        </ul>
      ...
      <button
        ...
        className="title-card-external-link-btn w-fit"
      >
